...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
    ...
    nav_msgs::Odometry des_state;
    nav_msgs::Odometry des_state_next;
    std:std::vector<nav_msgs::Odometry> new_state;
  
    des_state.header = start_pose.header; //really, want to copy the frame_id
    des_state.pose.pose = start_pose.pose; //start from here
 	double x_next = vec_of_states[1].pose.pose.position.x;
	double y_next = vec_of_states[1].pose.pose.position.y;
    double t_ramp = speed_max_ / accel_max_;
    int npts_ramp = round(t_ramp / dt_);
    ...
    double ramp_up_dist = 0.5 * accel_max_ * t_ramp*t_ramp;
    double ramp_up_time = omega_max_/ alpha_max_;    
    double ramp_up_angle = 0.5 * alpha_max_ * ramp_up_time*ramp_up_time;
   

    double dx = x_start - x_next;
    double dy = y_start - y_next;
    ...
    //double x_des = x_start+ ramp_up_dist*cos(psi_des);
    //double y_des = y_start+ ramp_up_dist*sin(psi_des);
    ...
    if (travel_dist == 0){
    	...
    	// position of des_state will not change; only orientation and twist
    	double t = 0.0;
    	double accel = sgn(psi_des) * alpha_max_; //watch out for sign: CW vs CCW rotation
	...
	// stops moving
 	...
 	vec_of_states = new_state; // no move for you. 
